...
def fetch_weather_for_crags_staging(
    ...
    """
    Returns a DataFrame ready for INSERT into stg_weather_route
    """
    if max_points:
    ...
    if not crags:
        return pd.DataFrame(columns=[
            ...
            "crag_id","longitude","latitude","load_batch_id"
        ])
    ...
    for crag_id, lat, lon in crags:
        ...
        try:
            ...
            df = pd.DataFrame({
                ...
                "windspeed_mm": hourly.Variables(3).ValuesAsNumpy(),
            })
            ...
            df["windspeed_mm"] = pd.to_numeric(df["windspeed_mm"], errors="coerce").clip(lower=0)
            ...
            frames.append(df[[
                "date","precipitation_mm","temperature_c","relative_humidity_percentage",
                "crag_id","longitude","latitude","load_batch_id", "windspeed_mm",
            ]])
    ...
    if not frames:
        return pd.DataFrame(columns=[
            "date","precipitation_mm","temperature_c","relative_humidity_percentage", "windspeed_mm"
            "crag_id","longitude","latitude","load_batch_id"
        ])
